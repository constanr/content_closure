...
class Embeddings:
    def __init__(self, vecsfile, vocabpath=None, normalize=True):
        if vocabpath is None: vocabpath = vecsfile.replace("npy", "vocab")
        self._vecs = np.load(vecsfile)
        with open(vocabpath) as vocabfile:
            self._vocab = vocabfile.read().split()
        if normalize:
    ...
    def similarity(self, word1, word2):
        w1 = self._vocab.index(word1)
        w2 = self._vocab.index(word2)
